...
Pull this repo and run the `install.sh` script. It assumes that you have a `usr/local/bin` in your `PATH` for the `note` cammand itself, a `/usr/local/etc/bash_completion.d` directory for bash completion, and/or a `~/.zsh/completions` directory for zsh completion.
